#!/usr/bin/env python3
"""
Expanded ETF/equities rotation research harness (V28).

Research goals
--------------
- Preserve frozen V26 as the paper-forward benchmark; V28 is separate research.
...
PACK_DIRNAME = "EquityRotationV28_research_pack"
...
def build_matrices(frames: Mapping[str, pd.DataFrame]) -> MarketMatrices:
    if "SPY" not in frames:
        raise ValueError("SPY data is required for V28 regime control.")
...
def target_weights(
    ...
    # V28 core-anchor overlay. During risk mode, reserve half of total
...
def development_splits(eval_dates: pd.DatetimeIndex):
    if len(eval_dates) < 1000:
        raise ValueError("Need at least 1000 evaluation sessions for V28.")
    n = len(eval_dates)
    dev_n = int(n * 0.80)
...
def evaluate_grid(matrices: MarketMatrices, trade_start_index: int):
    ...
    print(f"Testing {len(configs)} V28 ETF configurations...")
...
def save_pack(
    ...
    summary = {
        "strategy": "EQUITY_ROTATION_V28_RESEARCH",
        "status": status,
    ...
    (output_dir / "equity_rotation_v28_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True, default=str),
        encoding="utf-8",
    )

    readme = f"""# Equity Rotation V28 Research Pack

Status: **{status}**

This is research only. No orders were placed.

V28 is separate from frozen V26. It tests an expanded ETF universe using
...
def parse_args():
    p = argparse.ArgumentParser(description="Research expanded ETF/equities rotation V28.")
...
def main() -> int:
    args = parse_args()
    print("EQUITY ROTATION V28 RESEARCH — NO ORDERS WILL BE PLACED")
    ...
    if "SPY" not in active:
        raise SystemExit("SPY is unavailable; cannot run V28.")
    ...
    if not len(valid_spy):
        raise SystemExit("Not enough SPY history for V28 warmup.")
    trade_start = max(WARMUP_DAYS, int(valid_spy[0]) + 1)
    if len(matrices.dates) - trade_start < 1000:
        raise SystemExit("Not enough post-warmup sessions for four-fold V28 evaluation.")
