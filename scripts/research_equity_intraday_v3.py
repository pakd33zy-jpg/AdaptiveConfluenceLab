...
VERSION = "INTRADAY_V3"
OUT_DIR = Path("EquityIntradayV3_research_pack")
OUT_ZIP = Path("EquityIntradayV3_research_pack.zip")
...
def config_grid() -> list[Config]:
    ...
    return [
        ...
        for x in itertools.product(
            ...
            (3, 6),         # momentum bars
            (0.10, 0.15, 0.25),   # momentum threshold
            (1.10, 1.25, 1.50),   # relative volume
            (0.75, 1.00, 1.25),   # anti-chase range
            (1.0,),         # ATR stop
...
# V3 dynamically ranks where activity is concentrated instead of staring at a fixed watchlist.
    ...
    for symbol, x in featured.items():
        for i in range(1, len(x)):
            ...
            event = {
                ...
                "entry_signal": bool(prev["signal_long"]) and bool(spy_ok.get(prev["time"], False)) and (symbol in dynamic_leaders.get(prev["time"], set())),
