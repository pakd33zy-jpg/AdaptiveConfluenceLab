#!/usr/bin/env python3
"""
Expanded ETF/equities rotation research harness (V29).

Research goals
--------------
- Preserve frozen V26 as the paper-forward benchmark; V29 is separate research.
...
RISK_ON_CONFIRM_DAYS = 3
MIN_RISK_ON_BREADTH = 4
PACK_DIRNAME = "EquityRotationV29_research_pack"
...
CORE_ANCHORS = frozenset(("SPY", "QQQ", "RSP"))
CORE_ANCHOR_FRACTION = 0.25
...
def build_matrices(frames: Mapping[str, pd.DataFrame]) -> MarketMatrices:
    if "SPY" not in frames:
        raise ValueError("SPY data is required for V29 regime control.")
...
def target_weights(
    ...
    # V29 core-anchor overlay. During risk mode, reserve half of total
    # exposure for the strongest eligible broad-market anchor (SPY/QQQ/RSP).
    # The remaining half stays diversified across the selected satellites.
    if mode == "risk":
        anchor_candidates = [
            j for j in eligible.tolist()
            if matrices.symbols[j] in CORE_ANCHORS
        ]
        if anchor_candidates:
            best_anchor = max(anchor_candidates, key=lambda j: float(mom[j]))
            satellite = weights.copy()
            satellite[best_anchor] = 0.0
            satellite_sum = float(satellite.sum())
            weights[:] = 0.0
            weights[best_anchor] = exposure * CORE_ANCHOR_FRACTION
            if satellite_sum > 0.0:
                weights += (
                    satellite / satellite_sum
                    * exposure
                    * (1.0 - CORE_ANCHOR_FRACTION)
                )
            else:
                weights[best_anchor] = exposure
...
def development_splits(eval_dates: pd.DatetimeIndex):
    if len(eval_dates) < 1000:
        raise ValueError("Need at least 1000 evaluation sessions for V29.")
    n = len(eval_dates)
    dev_n = int(n * 0.80)
...
def evaluate_grid(matrices: MarketMatrices, trade_start_index: int):
    ...
    print(f"Testing {len(configs)} V29 ETF configurations...")
...
def save_pack(
    ...
    summary = {
        "strategy": "EQUITY_ROTATION_V29_RESEARCH",
        "status": status,
    ...
    (output_dir / "equity_rotation_v29_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True, default=str),
        encoding="utf-8",
    )

    readme = f"""# Equity Rotation V29 Research Pack

Status: **{status}**

This is research only. No orders were placed.

V29 is separate from frozen V26. It tests an expanded ETF universe using
...
def parse_args():
    p = argparse.ArgumentParser(description="Research expanded ETF/equities rotation V29.")
...
def main() -> int:
    args = parse_args()
    print("EQUITY ROTATION V29 RESEARCH — NO ORDERS WILL BE PLACED")
    ...
    if "SPY" not in active:
        raise SystemExit("SPY is unavailable; cannot run V29.")
    ...
    if not len(valid_spy):
        raise SystemExit("Not enough SPY history for V29 warmup.")
    trade_start = max(WARMUP_DAYS, int(valid_spy[0]) + 1)
    if len(matrices.dates) - trade_start < 1000:
        raise SystemExit("Not enough post-warmup sessions for four-fold V29 evaluation.")
