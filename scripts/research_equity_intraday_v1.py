...
VERSION = "INTRADAY_V1"
OUT_DIR = Path("EquityIntradayV1_research_pack")
OUT_ZIP = Path("EquityIntradayV1_research_pack.zip")
...
def config_grid() -> list[Config]:
    # Deliberately modest grid to reduce selection bias and Actions runtime.
    return [
        Config(*x)
        for x in itertools.product(
            ...
            (24, 36),      # slow EMA
            (3, 6),        # momentum bars
            (0.05, 0.10),  # minimum momentum over lookback, percent
            (1.0,),        # relative volume
            (1.0,),        # ATR stop
            (1.25, 1.75),  # reward/risk
...
def add_features(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # Signals are evaluated only on a completed bar. Entry is the NEXT bar open.
    x["signal_long"] = (
    ...
    for symbol, raw in history.items():
        ...
        for i in range(1, len(x)):
            ...
            event = {
                ...
                "signal_strength": float(prev["momentum_pct"]) if np.isfinite(prev["momentum_pct"]) else -np.inf,
            }
            events.setdefault(row["time"], []).append(event)
    return events

def backtest(history: dict[str, pd.DataFrame], cfg: Config, compound: bool = True, start_capital: float = STARTING_CAPITAL):
    events = prepare_events(history, cfg)
    cash = float(start_capital)
    position = None
    trades = []
    equity_rows = []
    fee_rate = cfg.fee_bps_per_side / 10000.0
    slip_rate = cfg.slippage_bps_per_side / 10000.0
    ...
    for ts in sorted(events):
        ...
        if position is None:
            candidates = [
                r for r in rows
                if r["entry_signal"] and np.isfinite(r["signal_atr"]) and r["signal_atr"] > 0
            ]
            if candidates:
...
def select_config(history: dict[str, pd.DataFrame]):
    ...
    for cfg in config_grid():
        ...
        robust = (
            min(fold_returns) > -5.0
            and max(abs(x) for x in fold_dds) <= 25.0
            and min(fold_pfs) >= 0.90
...
def main():
    ...
    summary = {
        ...
        "methodology": {
            ...
            "costs": "1 bp fee + 2 bp slippage per side",
        },
    }
    ...
    readme = f"""# Equity Intraday Research V1
