...
VERSION = "INTRADAY_V4"
OUT_DIR = Path("EquityIntradayV4_research_pack")
...
def build_dynamic_leader_map(history: dict[str, pd.DataFrame]) -> dict[pd.Timestamp, set[str]]:
    ...
    for ts, g in panel.groupby("time", sort=False):
        ...
        g = g[g["dollar_volume"] >= MIN_DOLLAR_VOLUME]
        if g.empty:
            continue
        ret_std = g["ret_15m"].std(ddof=0) + 1e-12
        vol_std = g["vol_ratio_local"].std(ddof=0) + 1e-12
        liq = np.log1p(g["dollar_volume"])
        liq_std = liq.std(ddof=0) + 1e-12
        score = (
            (g["ret_15m"] - g["ret_15m"].mean()) / ret_std
            + 0.8 * (g["vol_ratio_local"] - g["vol_ratio_local"].mean()) / vol_std
            + 0.35 * (liq - liq.mean()) / liq_std
        )
...
def add_features(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # V4 score: components can compensate for each other. We keep only a few
    # hard safety constraints (time, price elsewhere, valid ATR, not absurdly extended).
    x["score_trend"] = np.where(x["ema_fast"] > x["ema_slow"], 1.0, 0.0)
    x["score_vwap"] = np.clip((x["close"] - x["vwap"]) / x["atr"], -1.0, 1.0)
    x["score_momentum"] = np.clip(
        x["momentum_pct"] / max(cfg.min_momentum_pct, 1e-9), -2.0, 2.0
    )
    x["score_volume"] = np.clip(
        x["volume_ratio"] / max(cfg.min_volume_ratio, 1e-9), 0.0, 2.0
    )
    x["score_breakout"] = np.where(x["close"] > x["opening_range_high"], 1.0, 0.0)

    # Weighted favorability score, roughly 0-100 in normal conditions.
    raw = (
        18.0 * x["score_trend"]
        + 18.0 * np.clip(x["score_vwap"], 0.0, 1.0)
        + 24.0 * np.clip(x["score_momentum"], 0.0, 1.5) / 1.5
        + 20.0 * np.clip(x["score_volume"], 0.0, 1.5) / 1.5
        + 20.0 * x["score_breakout"]
    )
    x["setup_score"] = np.clip(raw, 0.0, 100.0)

    trade_window = x["minute_of_day"].between(600, 900)  # 10:00–15:00 ET
    x["signal_long"] = (
        trade_window
        & (x["atr"] > 0)
        & (x["vwap_extension_atr"] >= -0.25)
...
def prepare_events(history: dict[str, pd.DataFrame], cfg: Config) -> dict[pd.Timestamp, list[dict]]:
    ...
    dynamic_leaders = build_dynamic_leader_map(history)
    ...
    if spy is not None:
        for _, row in spy.iterrows():
            s = 0.0
            if np.isfinite(row["vwap"]) and row["close"] > row["vwap"]:
                s += 6.0
            if row["ema_fast"] > row["ema_slow"]:
                s += 6.0
            if np.isfinite(row["momentum_pct"]) and row["momentum_pct"] > 0:
                s += 3.0
            spy_score[row["time"]] = s
    ...
    for symbol, x in featured.items():
        for i in range(1, len(x)):
            ...
            leader_bonus = 12.0 if symbol in dynamic_leaders.get(prev["time"], set()) else 0.0
            market_bonus = float(spy_score.get(prev["time"], 0.0))
            total_score = float(prev["setup_score"]) + leader_bonus + market_bonus
            ...
            event = {
                ...
                "entry_signal": bool(prev["signal_long"]),
...
def backtest(history: dict[str, pd.DataFrame], cfg: Config, compound: bool = True, start_capital: float = STARTING_CAPITAL, cost_mult: float = 1.0):
    ...
    for ts in sorted(events):
        ...
        if position is None:
            candidates = [
                r for r in rows
                if r["entry_signal"] and r.get("setup_score", 0.0) >= cfg.min_setup_score and np.isfinite(r["signal_atr"]) and r["signal_atr"] > 0 and cfg.min_entry_price <= r["open"] <= cfg.max_entry_price
            ]
            if candidates:
...
def main():
    ...
    summary = {
        ...
        "methodology": {
            ...
            "v4_entry": "weighted favorability score using trend, VWAP, momentum, relative volume, opening-range behavior, dynamic-leader bonus, and SPY market bonus; only safety constraints are hard filters",
        },
    }
