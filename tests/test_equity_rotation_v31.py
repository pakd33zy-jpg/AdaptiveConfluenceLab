...
def test_full_backtest_each_family():
    ...
    for fam in m.FAMILIES:
        c=next(c for c in m.configs() if c.risk_profile==fam); r=m.base.run_backtest(mat,c,trade_start_index=320); assert np.isfinite(r.equity["equity"]).all(); assert (r.equity["equity"]>0).all()
