...
spec = importlib.util.spec_from_file_location("crypto_v4", MODULE_PATH)
...
def test_v4_excludes_cash_like_bases_and_has_no_fixed_liquidity_floor():
...
def test_grid_size_is_288():
    size = (
        ...
        * len(mod.REGIME_PROFILE_GRID) * len(mod.VOL_TARGET_GRID)
    )
    assert size == 288
...
def test_full_segment_return_uses_100k_anchor():
    matrices = mod.build_matrices(synthetic_frames())
    start = mod.WARMUP_DAYS
    result = mod.run_backtest(matrices, config(), trade_start_index=start)
...
def test_target_weights_respect_exposure_and_diversification():
    matrices = mod.build_matrices(synthetic_frames())
    weights, regime, vol, exposure = mod.target_weights(
        matrices, 500, config(weighting="equal")
    )
...
def test_strict_profile_is_cash_outside_up_regime():
    assert mod.REGIME_PROFILES["strict"]["sideways"] == 0.0
    assert mod.REGIME_PROFILES["strict"]["down"] == 0.0
    assert 0.0 < mod.REGIME_PROFILES["strict"]["up"] <= 1.0

def test_defensive_profile_has_small_sideways_exposure_and_zero_down():
    assert mod.REGIME_PROFILES["defensive"]["down"] == 0.0
    assert mod.REGIME_PROFILES["defensive"]["sideways"] == 0.10
...
def test_volatility_scaling_never_increases_regime_exposure():
    matrices = mod.build_matrices(synthetic_frames())
    c = config(regime_profile="defensive", vol_target_pct=35.0)
    exposure, regime, vol = mod.exposure_for_signal(matrices, 600, c)
    assert exposure <= mod.REGIME_PROFILES["defensive"][regime] + 1e-12
    assert 0 <= exposure <= 1

def test_breadth_filter_requires_four_eligible_assets():
    frames = synthetic_frames()
    for symbol in ("DOGE/USD", "AVAX/USD"):
        n = len(frames[symbol])
        close = 100.0 * np.cumprod(np.full(n, 0.998))
        frames[symbol]["close"] = close
        frames[symbol]["open"] = np.r_[close[0], close[:-1]]
    matrices = mod.build_matrices(frames)
    weights, regime, vol, exposure = mod.target_weights(matrices, 500, config())
    assert np.count_nonzero(weights) == 0

def test_development_splits_are_four_non_overlapping_folds_plus_diagnostic():
    dates = pd.date_range("2020-01-01", periods=1000, freq="D", tz="UTC")
    folds, diag = mod.development_splits(dates)
    assert len(folds) == 4
    assert folds[0][0] == dates[0]
    for left, right in zip(folds, folds[1:]):
        assert left[1] < right[0]
    assert folds[-1][1] < diag[0]
    assert diag[1] == dates[-1]

def test_backtest_equity_stays_finite_positive():
    matrices = mod.build_matrices(synthetic_frames())
    result = mod.run_backtest(
        matrices, config(), trade_start_index=mod.WARMUP_DAYS
    )
