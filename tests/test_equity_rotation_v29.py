...
MODULE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "research_equity_rotation_v29.py"
spec = importlib.util.spec_from_file_location("equity_v29", MODULE_PATH)
...
def test_v29_core_anchor_configuration():
    assert mod.RISK_ON_CONFIRM_DAYS >= 3
    assert mod.CORE_ANCHORS == frozenset(("SPY", "QQQ", "RSP"))
    assert 0.0 < mod.CORE_ANCHOR_FRACTION < 1.0


def test_v29_reduced_core_anchor():
    assert mod.CORE_ANCHOR_FRACTION == 0.25
    assert mod.RISK_ON_CONFIRM_DAYS >= 3
